...
download() {
  ...
  [[ ! -f "$output" ]] || { log "已存在: $output"; return; }
  log "下载: $url"
...
download_rpms() {
  ...
  if command -v dnf >/dev/null 2>&1; then
    ...
    die "RPM 离线包只能在带 yum/dnf、且与目标机同发行版/版本/架构的联网主机上准备"
  fi
}
...
main() {
  ...
  write_manifest
  log "离线依赖已准备到 $PACKAGE_DIR；可整体打包该目录，离线安装前运行 sha256sum -c packages/SHA256SUMS"
}
main "$@"
